...
export function WhatsAppFab() {
  return (
    <a
      href={waLink(WA_MENSAJES.general, "fab")}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Escríbenos por WhatsApp"
      className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-green-800 text-cream-50 shadow-flotante transition-transform duration-200 hover:scale-105 active:scale-95"
    >
      {IconoWA}
    </a>
  );
...
export function IconoWhatsApp({ className = "h-5 w-5" }: { className?: string }) {
  return <span className={`inline-flex ${className}`}>{IconoWA}</span>;
}
